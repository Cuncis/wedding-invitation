...
<html lang="id">
    ...
    <body class="bg-white text-slate-800 antialiased">
        ...
        {{-- ===================== TEMA ===================== --}}
        <section id="tema" class="py-24 bg-linear-to-b from-white to-rose-50/40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @php
                    $themes = [
                        [
                            'name' => 'Classic',
                            'image' => 'images/themes/classic.svg',
                            'desc' => 'Tema klasik abadi dengan ornamen emas dan monogram serif yang anggun.',
                            'price' => 'Rp 89.000',
                            'badge' => 'Default',
                            'badge_class' => 'text-amber-700 bg-amber-50 ring-amber-200',
                            'dots' => ['bg-amber-300', 'bg-stone-100 ring-1 ring-stone-200', 'bg-stone-700'],
                        ],
                        [
                            'name' => 'Elegant Rose',
                            'image' => 'images/themes/elegant-rose.svg',
                            'desc' => 'Floral romantis dengan nuansa pink rose dan tipografi serif elegan.',
                            'price' => 'Rp 99.000',
                            'badge' => 'Terlaris',
                            'badge_class' => 'text-rose-700 bg-rose-50 ring-rose-200',
                            'dots' => ['bg-rose-400', 'bg-rose-200', 'bg-amber-700'],
                        ],
                        [
                            'name' => 'Modern Minimalist',
                            'image' => 'images/themes/modern-minimalist.svg',
                            'desc' => 'Desain bersih dengan white space luas dan tipografi modern sans-serif.',
                            'price' => 'Rp 119.000',
                            'badge' => null,
                            'badge_class' => '',
                            'dots' => ['bg-slate-900', 'bg-slate-100 ring-1 ring-slate-200', 'bg-amber-400'],
                        ],
                        [
                            'name' => 'Floral Garden',
                            'image' => 'images/themes/floral-garden.svg',
                            'desc' => 'Tema kebun bunga dengan ilustrasi botanical dan palet hijau alami.',
                            'price' => 'Rp 109.000',
                            'badge' => null,
                            'badge_class' => '',
                            'dots' => ['bg-green-700', 'bg-stone-100 ring-1 ring-stone-200', 'bg-amber-400'],
                        ],
                    ];
                @endphp
                ...
                <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($themes as $theme)
                        <div class="group flex flex-col bg-white rounded-2xl overflow-hidden ring-1 ring-slate-100 shadow-sm hover:shadow-xl hover:shadow-rose-100/70 hover:ring-rose-100 transition-all duration-300 hover:-translate-y-1.5">
                            <div class="relative overflow-hidden bg-slate-50">
                                <img src="{{ asset($theme['image']) }}" alt="Tema {{ $theme['name'] }}" loading="lazy" class="w-full aspect-4/3 object-cover group-hover:scale-105 transition-transform duration-500">
                                @if ($theme['badge'])
                                    <span class="absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-semibold ring-1 backdrop-blur-sm {{ $theme['badge_class'] }}">{{ $theme['badge'] }}</span>
                                @endif
                                <div class="absolute inset-0 bg-linear-to-t from-slate-900/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-5">
                                    <a href="{{ route('register') }}" class="px-5 py-2 bg-white/95 text-rose-600 text-sm font-semibold rounded-full shadow-sm translate-y-2 group-hover:translate-y-0 transition-transform duration-300">Pilih Tema →</a>
                                </div>
                            </div>
                            <div class="flex flex-col flex-1 p-5">
                                <div class="flex items-center gap-1.5 mb-3">
                                    @foreach ($theme['dots'] as $dot)
                                        <span class="w-3.5 h-3.5 rounded-full inline-block {{ $dot }}"></span>
                                    @endforeach
                                </div>
                                <h3 class="font-serif text-lg font-semibold text-slate-900">{{ $theme['name'] }}</h3>
                                <p class="text-slate-500 text-sm mt-1.5 mb-4 leading-relaxed flex-1">{{ $theme['desc'] }}</p>
                                <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                                    <span class="font-bold text-slate-900">{{ $theme['price'] }}</span>
